...
try {
    $query = 'SELECT CompraA.Cantidad,CompraA.Total,CompraA.Confirmacion,Articulo.Nombre FROM CompraA INNER JOIN Articulo ON CompraA.IDArticulo = Articulo.IDArticulo WHERE CompraA.IDUsuario = :idcnta';
...
<html lang="en">
    <head>
        ...
        <title>Mis articulos comprados</title>
    </head>
    <body style="margin-top:5em">
        ...
        <main class="container">
            <div class="jumbotron jumbotron-fluid">
                <h2 class="container text-center">Historial de compras de articulos turisticos</h2>          
            </div>
             <ul class="list-group">
                <?php
                    for ($i = $numero - 1; $i >= 0; $i--) {
                        echo '<li class="list-group-item list-group-item-warning">'.$result[$i]['Nombre'].' | Cantidad:'.$result[$i]['Cantidad'].' | Costo de la compra: $'.$result[$i]['Total'].' MXN | Numero de confirmacion:'.$result[$i]['Confirmacion'].'</li>';
                    }
                ?>  
            </ul> 
        </main>
