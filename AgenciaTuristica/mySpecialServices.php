...
try {
    $query = 'SELECT mostrarCSE.Nombre,mostrarCSE.Cantidad,mostrarCSE.Total,mostrarCSE.Confirmacion,Reserva.Confirmacion AS confReserva FROM mostrarCSE'
            . ' INNER JOIN Reserva ON Reserva.IDReserva = mostrarCSE.IDReserva WHERE Reserva.IDUsuario = :idcnta ORDER BY mostrarCSE.IDContratacionSE';
    ...
    <body style="margin-top:5em">
        ...
        <main class="container">
            <div class="jumbotron jumbotron-fluid">
                <h2 class="container text-center">Historial de contrataciones de servicios especiales turisticos</h2>          
            </div>
             <ul class="list-group">
                <?php
                    for ($i = $numero - 1; $i >= 0; $i--) {
                        echo '<li class="list-group-item list-group-item-danger">'.$result[$i]['Nombre'].' | Numero de confirmacion:'.$result[$i]['Confirmacion'].
                                ' | Cantidad:'.$result[$i]['Cantidad'].' | Costo de la contratacion: $'.$result[$i]['Total'].' MXN | Numero de confirmacion de la reserva ligada:'
                                .$result[$i]['confReserva'].'</li>';
                    }
                ?>  
            </ul> 
        </main>
